...
import { floatSymbolApi } from '../api/FloatSymbolApi';
import EventSynchronizer from '../api/EventSynchronizer';

const useFloatSymbol = ({ componentId, symbolId }: SymbolProps) => {
  ...
  const writeValue = (newValue: number | null, isOriginalEvent?: boolean) => {
    ...
    if (newValue === 0 && isOriginalEvent === false) {
      ...
      setValue(newValue);
      setTimeout(() => floatSymbolApi.getValue(componentId, symbolId).then(setValue));
      return;
    }
    eventSynchronizer.current.valueBackup = newValue;
    setValue(newValue);
    ...
    if (typeof result === 'number') {
      ...
      floatSymbolApi.setValue(componentId, symbolId, result);
    }
  };
