...
const IntegerSymbolApi_1 = require("../api/IntegerSymbolApi");
const EventSynchronizer_1 = __importDefault(require("../api/EventSynchronizer"));
const useIntegerSymbol = ({ componentId, symbolId }) => {
    ...
    const initFromDB = () => {
        IntegerSymbolApi_1.integerSymbolApi
    ...
    const writeValue = (newValue, isOriginalEvent) => {
        ...
        if (newValue === 0 && isOriginalEvent === false) {
            ...
            setValue(newValue);
            setTimeout(() => IntegerSymbolApi_1.integerSymbolApi.getValue(componentId, symbolId).then(setValue));
            return;
        }
        eventSynchronizer.current.valueBackup = newValue;
        setValue(newValue);
        ...
        if (typeof result === 'number') {
            IntegerSymbolApi_1.integerSymbolApi.setValue(componentId, symbolId, result);
        }
    };
...
exports.default = useIntegerSymbol;
