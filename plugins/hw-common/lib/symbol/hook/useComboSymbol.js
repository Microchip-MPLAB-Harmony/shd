...
const ComboSymbolApi_1 = require("../api/ComboSymbolApi");
const EventSynchronizer_1 = __importDefault(require("../api/EventSynchronizer"));
const useComboSymbol = ({ componentId, symbolId }) => {
    ...
    const initFromDB = () => {
        ComboSymbolApi_1.comboSymbolApi.getComboSymbol(componentId, symbolId).then((symbol) => {
    ...
    const writeValue = (newValue) => {
        ...
        if (typeof result === 'string') {
            ComboSymbolApi_1.comboSymbolApi.setValue(componentId, symbolId, result);
        }
    };
...
exports.default = useComboSymbol;
